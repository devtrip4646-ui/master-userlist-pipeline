...
through 2026-09-13 (same definition as fd_users_retention_report() in
build_deposit_report.py, INCLUDING the new lossback_amount /
lossback_pct_of_total_bonus fields). Writes debug/fd_retention_sep7_13.json
and commits it back to the repo. One-off; delete this script and its
workflow after use.
"""
...
def main():
    ...
    with open(os.path.join(out_path, "fd_retention_sep7_13.json"), "w") as f:
    ...
    subprocess.run(["git", "add", "debug/fd_retention_sep7_13.json"], check=True)
    subprocess.run(["git", "commit", "-m", "debug: FD retention Sep 7-13", "--allow-empty"], check=True)
