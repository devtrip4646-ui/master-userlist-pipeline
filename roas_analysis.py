"""
Read-only one-off: ROAS analysis for the last 30 days.
Cohort = users whose FIRST-EVER deposit (is_first_deposit=1) landed within
the last 30 days. Computes gross deposit revenue and net revenue (deposit -
withdrawal) generated by that cohort so far, plus a channel-level breakdown
for context (spend isn't split by channel, so this is a blended/aggregate
ROAS, not per-channel attribution).
"""
import json
import os
import sqlite3
...
WINDOW_START = (TODAY - timedelta(days=29)).isoformat()  # 30-day window inclusive of today
...
# Identify the new-user cohort: first-ever deposit landed in the window.
new_user_channel = {}
new_user_ftd_amount = {}
new_user_ftd_date = {}
for user_id, amount, create_time, status, channel, is_first in all_deposits:
    ...
    if WINDOW_START <= d <= WINDOW_END:
        ...
        new_user_ftd_date[user_id] = d

cohort = set(new_user_channel.keys())

# All deposits/withdrawals made BY this cohort, within the window (their
# activity so far since acquisition -- includes the FTD itself plus any
# repeat deposits/withdrawals up to today).
...
# Per-channel breakdown (using each user's FTD channel as their acquisition channel)
...
for ch, row in by_channel.items():
    channel_rows.append({
        "channel": ch,
        "users": row["users"],
        "ftd_amount": round(row["ftd_amount"], 2),
        "total_deposit": round(row["total_deposit"], 2),
...
# VIP distribution reached so far by this cohort (early signal of quality)
